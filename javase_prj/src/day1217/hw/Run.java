			...
			try {
					...
					if (input.length == 3) {
						...
						try {
							...
							if ((javaScore > -1 && javaScore < 101) && (oracleScore > -1 && oracleScore < 101)) {
								...
								JOptionPane.showMessageDialog(null, "점수는 0~100사이값만 입력가능합니다.");
							}
				...
				if (Integer.parseInt(menu) == 2) {
					if (listData.isEmpty()) {
						...
						jta.append("번호\t이름\t오라클\t총점\t평균\n");
						for (int i = 0; i < listData.size(); i++) {
							javaScore = listData.get(i).getJavaScore();
							oracleScore = listData.get(i).getOracleScore();
							jta.append((i + 1) + "\t" + listData.get(i).getName() + "\t" + javaScore + "\t"
									+ oracleScore + "\t" + (javaScore + oracleScore) + "\t"
