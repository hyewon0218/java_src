...
public class UseObjectStream {
	...
	public void useObjectOutputStream() throws IOException {
		...
		try {
			...
			oos. writeObject(ud);//객체를 직렬화하여 스트림에 쓴다.(mashalling)
	...
	public void useObjectInputStream() throws IOException ,ClassNotFoundException{
		...
		try {
			...
			//File(하드디스크)에 저장된 객체(JVM)를 읽기위한 스트림 열기
			ois=new ObjectInputStream(new FileInputStream(file));
			//스트림을 통해서 직렬화된 객체를 얻어 재조립(unma)
	...
	public static void main(String[] args) {
		...
		try {
			uos.useObjectOutputStream();//쓰기
		} catch (IOException e) {
			e.printStackTrace();
		}
		System.out.println("===================================================");
		try {
			uos.useObjectInputStream();//읽기
