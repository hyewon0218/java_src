   ...
   <div id="container">
      <div>
      	<strong>연산자</strong>
      			...
      			<div>
      				<strong>EL 사용하여 Parameter 얻기(param객체 사용)</strong>
      				<ul>
      						<li>이름 : <strong>
      						<!--empty: 대상이 null이거나 ''인 경우 비교할 때 사용  -->
      						<c:out value="${empty param.name?'아래 링크를 클릭하거나 폼에 입력해주세요':param.name}"/></strong></li>
      						<li>나이 : <strong>${param.age}</strong></li>
      						<!-- EL을 사용하여 출력하면 XSS취약점을 가지게 된다.
      						XSS(Cross Site Script) : 악의적인 목적의 사용자가 HTML Form Control이나 
      						QueryString에 JavaScipt Code를 작성하여 서버에서 실행 시키기 위한 취약점. 
      						JSTL의 c:out을 사용하면 XSS가실행되지 않고 그대로 보여진다.(XSS취약점 막기)
      						-->
      				</ul>
      				<%
      					String name="기미철";
      					//scope 객체에 할당한 갑은 EL에서 사용할 수 있다.
