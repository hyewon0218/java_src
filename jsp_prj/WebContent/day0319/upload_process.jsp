...
<div id="wrap">
   ...
   <!-- enctype="multipart/form-data"인 경우 parameter를 받을 수 없다. -->
	...
	if(isMultipart) {//파일업로드에 적합한 요청
		...
		String uploader="",age="";
		String fieldName="",fileName="",contentType="";//지역변수초기화
		long sizeInByte=0;
		
		while (iter.hasNext()) {
		    FileItem item = iter.next();

		    if (item.isFormField()) {//일반 HTML Form Control인지?<input type="file"이 아닌 것들>
		    	String controlName = item.getFieldName();
		        String controlValue = item.getString();
		        
		        if("uploader".equals(controlName)) {//변수를 앞에두지 않기!!(nullpointexciption뜨지 않게하기위해)
		        ...
		        contentType = item.getContentType();//업로드한 File의 종류
		        sizeInByte = item.getSize();//크기	
