	   ...
	   <table id="diaryTab">
	   	...
	   	//매월마다 끝나는 날짜가 다르기 때문에 무한루프를 사용한다.
			...
			case START_DAY:
				...
				int prevLastDate=cal.getActualMaximum(Calendar.DATE);//이전월의 마지막 일
				
				cal.set(Calendar.MONTH,nowMonth-1);
				//다시 현재월로 변경하여 1일에 맞는 공백을 출력
				int week=cal.get(Calendar.DAY_OF_WEEK);
				
				for(int blankTd=1; blankTd<week; blankTd++) {
				 %>
				<td class="blankTd"><%=prevMonth %>/<%=prevLastDate-week+blankTd+1 %></td><!--28-현재월의수(6)를빼고+1+1  -->
				 <%
				}//end for
			}//end switch
