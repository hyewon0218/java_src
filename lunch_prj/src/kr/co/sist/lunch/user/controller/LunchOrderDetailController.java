...
public class LunchOrderDetailController extends WindowAdapter implements ActionListener{
	...
	private void lunchOrder() {
		...
		String tel=jtfTel.getText().trim();
			...
			//전화번호는 3/3~4/4 총 13자리 이하 
			//replaceall-하이픈없애기-길이판단
			//하이픈없애고 integer parseint 
			//numberformatexception
			String[] arrTel=tel.split("-");
			try {
				if(arrTel.length==3) {//제대로된 전화번호
					...
					for( int i=0; i <arrTel.length; i++ ) {
						Integer.parseInt(arrTel[i]);//숫자로마 되어있는지
					}//end for
					////주문 수행 ////
					printOrder();
				}else {
	...
	private void printOrder() {	
		...
		try {
			...
			.append("---------------------------------------------------------\n")
			.append("요청사항 : ").append(lodv.getJtaAsk().getText()).append("\n")//////////
		...
		case JOptionPane.OK_OPTION:
			try {
				OrderAddVO oavo=new OrderAddVO(lodv.getJtfOrderName().getText(), 
						...
						lodv.getJtaAsk().getText(),lodv.getJbQuan().getSelectedIndex()+1);//-> +1해주면 selecteditem이랑 같음 /1-0,2-1
				
				try {
					LunchClientDAO.getInstance().insertOrder(oavo);//////////////////////////////
					showMessageDialog(lodv, "도시락주문이 완료되었습니다.");
					//주문이 완료되었으므로 
					lodv.dispose();
...
//		showMessageDialog(lodv, "주문할게요!");////x로닫았을때??
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==lodv.getJbQuan()) {//수량 선택시 전체 가격 보여준다.
			try {
				...
				showMessageDialog(lodv, "가격은 정수이어야합니다.");
			}//end catch
		}//end if
