...
<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 <%
	...
	if(dong !=null) {
 	...
 	<c:if test="${not empty param.dong }">
 		...
 		<table border="1">
 			...
 			<c:forEach var="zip" items="${zipList }">
 				<tr>
 					<td><a href=#void onclick= "setMarker('${zip.sido } ${zip.gugun } ${zip.dong }','${zip.sido }')" ><c:out value="${zip.zipcode }"/></a></td>
 					<td><a href=#void onclick= "setMarker('${zip.sido } ${zip.gugun } ${zip.dong }','${zip.sido }')" ><c:out value="${zip.zipcode }"/><c:out value="${zip.sido } ${zip.gugun } ${zip.dong } ${zip.bunji }"/></a></td>
 				</tr>
 			</c:forEach>
 		</table>
 	</c:if>
 	<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=8993007dc62063ebf51f248cff565a37&libraries=services"></script>
 	...
 	setMarker("서울시 동작구 상도동","살기 좋은 상도동");
 	});
