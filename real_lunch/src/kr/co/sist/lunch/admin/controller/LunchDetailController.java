...
public class LunchDetailController extends WindowAdapter implements ActionListener{
	...
	private void modifyLunch() {
		...
		// 선택한 이미지와 동일한 이름의 이미지가 s_를 가지지 않은 경우 이미지작업이 되어있지
		//않은 경우. uploadImg 를 사용
	...
	/**
	 * 큰이미지명을 가진 File객체를 입력하여 업로드 폴더에 큰이미지(m1_l1.gif)와 s_가
	 * 	 붙은 작은 이미지(s_m1_l1.gif)를 업로드 한다. 
	 * @param file
	 */
