...
public class WebMemberJoin extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out=response.getWriter();
		  ...
		  //사용자가 입력한 값(HTML Form Control) 받기(getParameter)
		  ...
		  //파라메터 값을 VO에 저장(비밀번호 암호화되어 들어감)
		  WebMemberVO wm_vo=new WebMemberVO(id, ShaUtil.shaEncoding(pass1), name, ShaUtil.shaEncoding(ssn1+"-"+ssn2), gender, b_year+"-"+b_month+"-"+b_day, 
				  age, marrigeFlag, m_year+" "+m_month+"-"+m_day, zipcode, addr1, addr2, greetings, request.getRemoteAddr(),interestlang);
	      ...
	      out.write("<link rel=\"stylesheet\" type=\"text/css\" href=\"http://localhost:8080/servlet_prj/common/css/main_v20190130.css\"/>\r\n");
	      ...
	      out.write("\t</div>\r\n");
	    	  ...
	    	  out.println("<a href='#void' onclick='history.back();'>다시입력</a><br/>");
	    	  
	      }catch(SQLException se) {
	    	  se.printStackTrace();
	    	  out.println("회원가입이 정상적으로 이루어지지 않았습니다<br/>");
	    	  out.println("서비스가 원활하지 못한 점 뎨동합니다.<br/>");
	    	  out.println("정상적인 서비스를 위해서 저희 임직원들은 최선을 다하고있습니다.<br/>");
	    	  out.println("신속하게 복구하겠습니다. 사용 중 불편한 점은 010-디잘잘에게 연락해주세요.<br/>");
	      }//end catch
	      out.write("\t\t\r\n");
	      out.write("\t</div>\r\n");
	      out.write("</div>\r\n");
	...
	private void addWebMember(WebMemberVO wm_vo)throws SQLException, RuntimeException{
		...
		//추가전에동일한 아이디가 존재하는지 다시 검색하여 처리(비연결성 처리)
