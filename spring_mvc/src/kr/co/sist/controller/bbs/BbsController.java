...
public class BbsController {
	...
	public String diaryList( DiaryVO dv, Model model) {
		...
		int totalCount=ds.totalCount();//총게시물의 수
		int pageScale=ds.pageScale();//한 화면에 보여줄 게시물의 수 
		int totalPage=ds.totalPage(totalCount);//총 게시물을 보여주기 위한 페이지수
		if(dv.getCurrentPage()==0) {//web parameter에 값이 없을 때 
			dv.setCurrentPage(1);//1번부터 조회해야하므로 1로 설정 
		}
	...
	public String bbsRead(int num, Model model) {
		...
		List<DiaryReply>replytList=ds.searchReplyDiaryList(num);//원글의 댓글들을 조회
